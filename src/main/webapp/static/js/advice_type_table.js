...
jQuery(document).ready(function () {
    oTable = $('#type_table').dataTable({
        ...
        "aoColumns": [
            {
                ...
                "mRender": function (val, data, full) {
                    var str = '';
                    if (!full.first) {
                        str += '<a href="#" onclick="modifyTypeOrderUp(' + full.id + ',\'' + full.sequence + '\')" class="btn"><i class="icon-long-arrow-up" ></i></a>';
                    }
                    if (!full.end) {
                        str += '<a href="#" onclick="modifyTypeOrderDown(' + full.id + ',\'' + full.sequence + '\')" class="btn"><i class=" icon-long-arrow-down" ></i></a>';
                    }
                    return str;
                }
            }, {
        ...
        "bProcessing": true,
        "bServerSide": true,
        "sAjaxSource": $ctx + "/advice/type/list",
...
function deleteType(id) {
    if (window.confirm("确定删除？")) {
        $.ajax({
            dataType: 'json',
            async: false,
            url: $ctx + "/advice/type/delete?id=" + id,
            // data: "{}",
            success: function (data) {
                reLoad(data);
            }
...
function modifyTypeOrderUp(id,sequence) {
    modifyType(id, null, true,sequence);
}

function modifyTypeOrderDown(id,sequence) {
    modifyType(id, null, false,sequence);

}

function modifyType(id, name, asc,sequence) {
    $.ajax({
        dataType: 'json',
        type: "POST",
        async: false,
        // contentType:'application/json',
        url: $ctx + "/advice/type/modify?id=" + id + "&name=" + name + "&asc=" + asc+ "&sequence=" + sequence,
        // data: data,
        success: function (data) {
            reLoad(data);
        }
